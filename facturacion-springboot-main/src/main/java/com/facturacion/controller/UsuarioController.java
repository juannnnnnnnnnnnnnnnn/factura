...
public class UsuarioController {
    ...
    public ResponseEntity<ResponseMessage> register(@RequestBody Usuario usuario) {
        try {
            if (usuarioService.existsByUsername(usuario.getUsername())) {
                return ResponseEntity.ok(new ResponseMessage(HttpStatus.BAD_REQUEST.value(), "Usuario ya existe"));
            }
            ...
            return ResponseEntity.ok(new ResponseMessage(HttpStatus.OK.value(), "Usuario registrado exitosamente"));
        } catch (Exception e) {
            return ResponseEntity.ok(new ResponseMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Error al registrar usuario"));
        }
    }
